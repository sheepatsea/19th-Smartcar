...
class Tracking
{
 public:
    ...
    vector<POINT> pointsEdgeLeft_Seed;     // 赛道左边缘点集
    vector<POINT> pointsEdgeRight_Seed;    // 赛道右边缘点集
    ...
    double stdevLeft_t;                 // 直弯道边缘斜率方差（左）
    double stdevRight_t;                // 边缘斜率方差（右）
 ...
 //   int left_num = 0, right_num = 0; 
    ...
    void trackRecognition(bool isResearch, uint16_t rowStart)
    {
        ...
        for (int row = rowStart; row > rowCutUp; row--) // 有效行：10~220
        {
            ...
            else if (imageType == ImageType::Binary) // 输入二值化图像
            {
            ...
            if (flagStartBlock)                            // 起始行做特殊处理
            {
                ...
                if (widthBlocks > limitWidthBlock) // 满足首行宽度要求
                {
                    ...
                    pointsEdgeLeft.emplace_back(row, startBlock[indexWidestBlock]);
                    pointsEdgeRight.emplace_back(row, endBlock[indexWidestBlock]);
            ...
            else // 其它行色块坐标处理
            {
                ...
                if(parkingsum<5)
                {
                    ...
                    if (counterBlock > 5)
                    {
                        ...
                        for (int i = 0; i < counterBlock; i++)
                        {
                            ...
                            if (widthThis > 5 && widthThis < 20 && endBlock[i] > pointsEdgeLeft[pointsEdgeLeft.size() - 1].y 
                                && startBlock[i] < pointsEdgeRight[pointsEdgeRight.size() - 1].y 
                                )            // 过滤无效色块区域：噪点
                            {
                        ...
                        if (widthGarage.size() >= 5) // 验证有效斑马线色块个数
                        {
                            parkingsum++;
                        }
                        if(parkingsum>=3)
                        {
                            if(!garageEnable.x) garageEnable.x = 1;                      // 车库标志使能
                            garageEnable.y = pointsEdgeRight[pointsEdgeRight.size()-1].x; // 斑马线行序号
                            //printf("%d\n", garageEnable.y );
                        }
                    } 
                }
                ...
                else if (indexBlocks.size() > 1) // 存在多个色块，则需要择优处理：选取与上一行最近的色块
                {
                    ...
                    pointsEdgeLeft.emplace_back(row, startBlockNear);
                    pointsEdgeRight.emplace_back(row, endBlockNear);
       ...
       // stdevLeft_t = stdevEdgeCal_t(pointsEdgeLeft, ROWSIMAGE); // 计算边缘方差
        //stdevRight_t = stdevEdgeCal_t(pointsEdgeRight, ROWSIMAGE);
         //   printf("%d\n",pointsEdgeLeft.size() );
        ...
        if (imageType == ImageType::Binary)                 ///////////////////
        {
            ...
            temp_counter_white = 240 - rowCutBottom - rowCutUp;
                for (int row = 239 - rowCutBottom; row >= rowCutUp; row --)
                {
                    if (imagePath.at<uchar>(row, col) <= 127)
                    {
                        temp_counter_white = 239 - rowCutBottom - row;
                        break;
                    }
                }
        ...
        // findline_maze(imagePath, &start_row_ipm, &left_edge_ipm, &right_edge_ipm, ipm_image, pointsEdgeLeft[0], pointsEdgeRight[0]);//二值化图种子生长
        
        // pointsEdgeLeft_Seed.clear();
        // for(int i=0;i<left_edge_ipm.real_size;i++)
        // {
        //     pointsEdgeLeft_Seed.emplace_back(left_edge_ipm.point[i].row, left_edge_ipm.point[i].col);
        // }
        // pointsEdgeRight_Seed.clear();
        // for(int i=0;i<right_edge_ipm.real_size;i++)
        // {
        //     pointsEdgeRight_Seed.emplace_back(right_edge_ipm.point[i].row, right_edge_ipm.point[i].col);
        // }
    }
...
//      * @return double
//      */
//     double stdevEdgeCal(vector<POINT> &Edge)
//     {
//         vector<int> X, Y;
...
//         }
//         double correlation = numerator / (std::sqrt(denominatorX) * std::sqrt(denominatorY));
//         return correlation;
...
//     }
   ...
   /**
    ...
    double stdevEdgeCal(vector<POINT> &v_edge, int img_height)
    {
        ...
        vector<int> v_slope;
        int step = 10; // v_edge.size()/10;
        for (int i = 80; i < v_edge.size(); i += step)
        {
            if (v_edge[i].x - v_edge[i - step].x)
                v_slope.push_back((v_edge[i].y - v_edge[i - step].y) * 100 / (v_edge[i].x - v_edge[i - step].x));
        }
...
double stdevEdgeCal_t(vector<POINT> &v_edge, int img_height)
{
    if (v_edge.size() < img_height / 4)
    {
        return 1000;
    ...
    vector<int> v_slope;
    int step = 2; // v_edge.size()/10;
    int top = (v_edge.size()-20<130)?v_edge.size()-20:130;
    for (int i = 80; i < top; i += step)
    {
        if (v_edge[i].x - v_edge[i - step].x)
            v_slope.push_back((v_edge[i].y - v_edge[i - step].y) * 100 / (v_edge[i].x - v_edge[i - step].x));
    }
