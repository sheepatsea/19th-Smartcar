...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    if(count_mode==COUNT) count_dis += Motor.speed * 0.001;
    if(count_mode==COUNT && count_dis >= exp_dis)
    {
       // count_mode = 0;
        //count_mode = NONE;
    ...
    Speed_Control();
    if(uart_wait>500) Motor.output  = 0;
    Speed_Output();
...
//    if((count_mode==RESCUE_WAITING && count_dis >= WAITING_DIS) ||
//            (count_mode==RESCUE_TURNING && count_dis >= TURNING_DIS))
//    {
//        count_mode = 0;
//        g_beep_set = 1;
//        USB_Edgeboard_TransmitKey();
