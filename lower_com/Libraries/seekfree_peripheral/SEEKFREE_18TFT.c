...
void lcd_showchar_co(uint16 x,uint16 y,const int8 dat,uint16 co)
{
    uint8 i,j;
    uint8 temp;
    ...
    for(i=0; i<16; i++)
    {
        ...
        for(j=0; j<8; j++)
        {
            if(temp&0x01)   lcd_writedata_16bit(co);
            else            lcd_writedata_16bit(TFT_BGCOLOR);
            temp>>=1;
        }
    }
...
//-------------------------------------------------------------------------------------------------------------------
void lcd_showuint16(uint16 x,uint16 y,uint16 dat,uint8 num)
{
	uint8 a[5];
	uint8 i;
	...
	{
		if(i == num)
		    lcd_showchar_co(x+(8*i),y*16,'0' + a[i], BLUE);
		else
		    lcd_showchar(x+(8*i),y*16,'0' + a[i]);
		i++;
	}
}
void lcd_showuint16_co(uint16 x,uint16 y,uint16 dat,uint16 co)
{
    uint8 a[5];
    uint8 i;
    ...
    while(i<5)
    {
        lcd_showchar_co(x+(8*i),y*16,'0' + a[i],co);
        i++;
    }
}
//-------------------------------------------------------------------------------------------------------------------
